...
func WhenCityWasRemoved(cityRepository persistence.CityRepository) eventbus.EventHandler {
	fn := func(parentCtx context.Context, event *domain.Event) error {
		ctx, cancel := context.WithTimeout(parentCtx, time.Second*120)
		defer cancel()

		e := event.Payload.(*city.WasRemoved)

		if err := cityRepository.Delete(ctx, e.CityID.String()); err != nil {
