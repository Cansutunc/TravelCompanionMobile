...
// WhenCompetitionWasCreated handles event
func WhenCompetitionWasCreated(repository persistence.CompetitionRepository) eventbus.EventHandler {
	fn := func(parentCtx context.Context, event *domain.Event) error {
		ctx, cancel := context.WithTimeout(parentCtx, time.Second*120)
		defer cancel()

		e := event.Payload.(*competition.WasCreated)
