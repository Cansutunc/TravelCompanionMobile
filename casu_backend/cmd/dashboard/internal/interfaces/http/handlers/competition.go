...
func BuildListCompetitionsHandler(repository persistence.CompetitionRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		...
		paginatedList := struct {
			Competitions []persistence.Competition `json:"cities"`
		...
		paginatedList.Competitions, err = repository.FindAll(r.Context(), limit, offset)
		if err != nil {
			return apperrors.Wrap(err)
		}
...
// BuildGetUserHandler
func BuildGetCompetitionHandler(repository persistence.CompetitionRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
...
func BuildCompetitionCommandDispatchHandler(cb commandbus.CommandBus) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		if r.Body == nil {
		...
		c, err := competition.NewCommandFromPayload(params.Value("command"), body)
		if err != nil {
			return apperrors.Wrap(err)
		}
