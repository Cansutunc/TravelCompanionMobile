...
func BuildListBooksHandler(repository persistence.BookRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		...
		paginatedList := struct {
			Books []persistence.Book `json:"data"`
		...
		paginatedList.Books, err = repository.FindAll(r.Context(), limit, offset)
		if err != nil {
			return apperrors.Wrap(err)
		}
...
// BuildGetUserHandler
func BuildGetBookHandler(repository persistence.BookRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
...
func BuildBookCommandDispatchHandler(cb commandbus.CommandBus) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		if r.Body == nil {
		...
		c, err := book.NewCommandFromPayload(params.Value("command"), body)
		if err != nil {
			return apperrors.Wrap(err)
		}
