...
func BuildListCountriesHandler(repository persistence.CountryRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		...
		paginatedList := struct {
			Countries []persistence.Country `json:"data"`
		...
		paginatedList.Countries, err = repository.FindAll(r.Context(), limit, offset)
		if err != nil {
			return apperrors.Wrap(err)
		}
...
// BuildGetUserHandler
func BuildGetCountryHandler(repository persistence.CountryRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
...
func BuildCountryCommandDispatchHandler(cb commandbus.CommandBus) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		if r.Body == nil {
		...
		c, err := country.NewCommandFromPayload(params.Value("command"), body)
		if err != nil {
			return apperrors.Wrap(err)
		}
