...
func BuildListMatchesHandler(repository persistence.MatchRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		...
		paginatedList := struct {
			Matches []persistence.Match `json:"matches"`
		...
		paginatedList.Matches, err = repository.FindAll(r.Context(), limit, offset)
		if err != nil {
			return apperrors.Wrap(err)
		}
...
// BuildGetUserHandler
func BuildGetMatchHandler(repository persistence.MatchRepository) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
...
func BuildMatchCommandDispatchHandler(cb commandbus.CommandBus) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) error {
		if r.Body == nil {
		...
		c, err := match.NewCommandFromPayload(params.Value("command"), body)
		if err != nil {
			return apperrors.Wrap(err)
		}
